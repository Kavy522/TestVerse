...
from utils.helpers import (
    ...
    calculate_exam_result, auto_evaluate_mcq
)
...
class StudentExamListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Filter by department if allowed_departments is set
        # Use Python-based filtering for SQLite compatibility (no JSON __contains support)
        filtered_exams = []
        for exam in exams:
            if not exam.allowed_departments or user.department in exam.allowed_departments:
...
class StudentExamDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        # Show all published exams that haven't ended yet
        # Students can view upcoming exams but can't start until start_time
...
class StudentSaveAnswerView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        question_id = request.data.get('question') or request.data.get('questionId')
